...
export function checkTextEquality(input, word) {
 ...
 export function create_answers(word, words, num_of_answers) {
   ...
   const word_index = words_copy.findIndex(curr_word => word._id ===  curr_word._id)
   words_copy.splice(word_index, 1)
   let answers = [word.meaning]
   const random_answers = pick_n_random(words_copy.map(w => w.meaning), num_of_answers-1)
   answers = shuffledArray([...answers, ...random_answers])
   const correct_answer_index = answers.findIndex(curr => curr === word.meaning)
